// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: recorder/provider.proto

// Protobuf Java Version: 3.25.3
package com.wecasino.proto.recorder;

/**
 * Protobuf enum {@code recorder.InstructionCmdType}
 */
public enum InstructionCmdType
    implements com.google.protobuf.ProtocolMessageEnum {
  /**
   * <pre>
   * 未指定
   * </pre>
   *
   * <code>NONE_INSTRUCTION_TYPE_UNSPECIFIED = 0;</code>
   */
  NONE_INSTRUCTION_TYPE_UNSPECIFIED(0),
  /**
   * <pre>
   * 包桌換靴
   * </pre>
   *
   * <code>INSTRUCTION_GROUP_CHANGE_SHOE = 1;</code>
   */
  INSTRUCTION_GROUP_CHANGE_SHOE(1),
  /**
   * <pre>
   * 包桌換荷官
   * </pre>
   *
   * <code>INSTRUCTION_GROUP_CHANGE_DEALER = 2;</code>
   */
  INSTRUCTION_GROUP_CHANGE_DEALER(2),
  /**
   * <pre>
   * 包桌開牌
   * </pre>
   *
   * <code>INSTRUCTION_GROUP_OPEN_CARD = 3;</code>
   */
  INSTRUCTION_GROUP_OPEN_CARD(3),
  UNRECOGNIZED(-1),
  ;

  /**
   * <pre>
   * 未指定
   * </pre>
   *
   * <code>NONE_INSTRUCTION_TYPE_UNSPECIFIED = 0;</code>
   */
  public static final int NONE_INSTRUCTION_TYPE_UNSPECIFIED_VALUE = 0;
  /**
   * <pre>
   * 包桌換靴
   * </pre>
   *
   * <code>INSTRUCTION_GROUP_CHANGE_SHOE = 1;</code>
   */
  public static final int INSTRUCTION_GROUP_CHANGE_SHOE_VALUE = 1;
  /**
   * <pre>
   * 包桌換荷官
   * </pre>
   *
   * <code>INSTRUCTION_GROUP_CHANGE_DEALER = 2;</code>
   */
  public static final int INSTRUCTION_GROUP_CHANGE_DEALER_VALUE = 2;
  /**
   * <pre>
   * 包桌開牌
   * </pre>
   *
   * <code>INSTRUCTION_GROUP_OPEN_CARD = 3;</code>
   */
  public static final int INSTRUCTION_GROUP_OPEN_CARD_VALUE = 3;


  public final int getNumber() {
    if (this == UNRECOGNIZED) {
      throw new java.lang.IllegalArgumentException(
          "Can't get the number of an unknown enum value.");
    }
    return value;
  }

  /**
   * @param value The numeric wire value of the corresponding enum entry.
   * @return The enum associated with the given numeric wire value.
   * @deprecated Use {@link #forNumber(int)} instead.
   */
  @java.lang.Deprecated
  public static InstructionCmdType valueOf(int value) {
    return forNumber(value);
  }

  /**
   * @param value The numeric wire value of the corresponding enum entry.
   * @return The enum associated with the given numeric wire value.
   */
  public static InstructionCmdType forNumber(int value) {
    switch (value) {
      case 0: return NONE_INSTRUCTION_TYPE_UNSPECIFIED;
      case 1: return INSTRUCTION_GROUP_CHANGE_SHOE;
      case 2: return INSTRUCTION_GROUP_CHANGE_DEALER;
      case 3: return INSTRUCTION_GROUP_OPEN_CARD;
      default: return null;
    }
  }

  public static com.google.protobuf.Internal.EnumLiteMap<InstructionCmdType>
      internalGetValueMap() {
    return internalValueMap;
  }
  private static final com.google.protobuf.Internal.EnumLiteMap<
      InstructionCmdType> internalValueMap =
        new com.google.protobuf.Internal.EnumLiteMap<InstructionCmdType>() {
          public InstructionCmdType findValueByNumber(int number) {
            return InstructionCmdType.forNumber(number);
          }
        };

  public final com.google.protobuf.Descriptors.EnumValueDescriptor
      getValueDescriptor() {
    if (this == UNRECOGNIZED) {
      throw new java.lang.IllegalStateException(
          "Can't get the descriptor of an unrecognized enum value.");
    }
    return getDescriptor().getValues().get(ordinal());
  }
  public final com.google.protobuf.Descriptors.EnumDescriptor
      getDescriptorForType() {
    return getDescriptor();
  }
  public static final com.google.protobuf.Descriptors.EnumDescriptor
      getDescriptor() {
    return com.wecasino.proto.recorder.ProviderProto.getDescriptor().getEnumTypes().get(1);
  }

  private static final InstructionCmdType[] VALUES = values();

  public static InstructionCmdType valueOf(
      com.google.protobuf.Descriptors.EnumValueDescriptor desc) {
    if (desc.getType() != getDescriptor()) {
      throw new java.lang.IllegalArgumentException(
        "EnumValueDescriptor is not for this type.");
    }
    if (desc.getIndex() == -1) {
      return UNRECOGNIZED;
    }
    return VALUES[desc.getIndex()];
  }

  private final int value;

  private InstructionCmdType(int value) {
    this.value = value;
  }

  // @@protoc_insertion_point(enum_scope:recorder.InstructionCmdType)
}

